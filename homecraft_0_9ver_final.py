...
def main(args=None):
    ...
    ROT_TABLE = {
         144: (146.28, 0.69, -8.44), 128: (128.60, 1.25, -8.14), 112: (112.55, 1.60, -8.10),
          96: (97.19, 1.90, -8.26),  80: (81.36, 2.52, -8.45),  64: (64.92, 1.54, -8.58),
          48: (50.50, 0.98, -8.57),  32: (33.45, 1.58, -8.92),  16: (17.56, 1.91, -8.83),
           0: (1.41, 1.90, -8.73),   -16: (-13.93, -0.03, -8.67), -32: (-30.77, 0.79, -8.48),
         -48: (-47.21, 1.62, -8.98),  -64: (-61.45, 2.29, -8.91), -80: (-78.57, 2.96, -8.39),
         -96: (-93.51, 4.12, -8.94), -112: (-111.44, -0.04, -8.61), -128: (-125.56, 0.66, -8.23),
    ...
    SLIDE_OFFSET, SLIDE_HOVER, SLIDE_FORCE, SLIDE_DOWN_FORCE, SLIDE_TIME = 4.0, 5.0, 5.0, 3.0, 2.5

    # 초기 구동 세팅
    open_gripper()
    movej(homej, vel=300, acc=300)
    ...
    try:
        while rclpy.ok():
            ...
            for current_block in lego_queue:
                ...
                # 3. 주변 상황 분석 (슬라이딩 조립 vs 코너 불도저 조립)
                ...
                # 2️⃣ [패턴 B] (신규) 양쪽 다 막힌 사각형의 마지막 코너 자리에 꽂을 때: 비틀기 진입
                    ...
                    set_desired_force([cx * 10.0, 0.0, 0, 0, 0, 0], dir=[1, 0, 0, 0, 0, 0], mod=DR_FC_MOD_REL)
                    wait(2.0)  
                    ...
                    set_desired_force([0.0, cy * 10.0, 0, 0, 0, 0], dir=[0, 1, 0, 0, 0, 0], mod=DR_FC_MOD_REL)
                    wait(2.0)  
                    release_force(); release_compliance_ctrl(); wait(0.1)
                ...
                # 3️⃣ [패턴 C] 주변에 아무것도 없을 때: 원래 쓰시던 정직한 첫 수직 놓기 로직
                    ...
                    # 드라이버처럼 그리퍼 회전
                    move_periodic(amp=[0, 0, 0, 0, 0, 2], period=2.0, repeat=1, ref=USER_COORD); wait(0.1)
                ...
                task_compliance_ctrl([4000, 4000, 20000, 200, 200, 200], 0); wait(0.2)
                set_desired_force([0, 0, 20, 0, 0, 0], dir=[0, 0, 1, 0, 0, 0], mod=DR_FC_MOD_REL)
