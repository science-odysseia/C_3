...
def main(args=None):
  ...
  # =========================================================================
    # [4-4. 아래층 지지 면적 계산 -- half-offset 낙하 방지]
    # =========================================================================
    BLOCK_HALF = 1.0  # 2x2 블록의 절반 폭 (grid 단위, 16mm의 절반)

    def get_footprint(gx, gy):
        """블록의 평면 범위를 grid 단위로 반환: (x_min, x_max, y_min, y_max)"""
        return (gx - BLOCK_HALF, gx + BLOCK_HALF, gy - BLOCK_HALF, gy + BLOCK_HALF)

    def compute_support(grid_x, grid_y, grid_z):
        """
        바로 아래층(grid_z - 1)에 배치된 블록들과의 겹침을 계산.
        반환값: (support_ratio, offset_x_grid, offset_y_grid)
          - support_ratio: 아래층에 지지되는 면적 비율 (0.0 ~ 1.0)
          - offset_x/y_grid: 블록 중심 대비 '지지 중심'의 편차 (grid 단위)
        아래층에 겹치는 블록이 없으면 (0.0, 0.0, 0.0) 반환.
        """
        x_min, x_max, y_min, y_max = get_footprint(grid_x, grid_y)
        ...
        for (bx, by, bz) in placed_blocks:
            if bz != grid_z - 1:
                continue
            bx_min, bx_max, by_min, by_max = get_footprint(bx, by)

            ox = max(0.0, min(x_max, bx_max) - max(x_min, bx_min))
            oy = max(0.0, min(y_max, by_max) - max(y_min, by_min))
            area = ox * oy

            if area > 0:
                cx = (max(x_min, bx_min) + min(x_max, bx_max)) / 2.0
                cy = (max(y_min, by_min) + min(y_max, by_max)) / 2.0
                overlap_area += area
                weighted_cx += cx * area
                weighted_cy += cy * area
        ...
        support_ratio = overlap_area / total_area
        offset_x = (weighted_cx / overlap_area) - grid_x
        offset_y = (weighted_cy / overlap_area) - grid_y
        return support_ratio, offset_x, offset_y
    ...
    try:
        for current_block in lego_queue:
            ...
            if use_slide:
                ...
                # 표면 10mm 위까지는 빠르게, 이후는 힘 제어로 서서히 접촉
                pose_place_hover = deepcopy(posx1)
                pose_place_hover[2] -= 10.0
                movel(pose_place_hover, vel=150, acc=ACC)
                wait(0.1)

                node.get_logger().info("힘 제어 접촉 하강: 표면까지 서서히 내림")
                task_compliance_ctrl([3000, 3000, 3000, 200, 200, 200], 0)
                wait(0.2)

                fd_touch = [0, 0, 8.0, 0, 0, 0]  #목표 힘(desired force) 벡터
                fc_touch = [0, 0, 1, 0, 0, 0] # 힘 제어를 적용할 축을 선택하는 마스크
                set_desired_force(fd_touch, dir=fc_touch, mod=DR_FC_MOD_REL) # 로봇의 현재 위치를 기준으로 상대적으로 z방향 8N이 될 때까지 서서히 내려갑
                wait(1.5)   # 접촉해서 힘 평형 도달할 때까지
            ...
            # -----------------------------------------------------------------
            # --- 지지율 계산 (half-offset 낙하 방지 핵심) ---
            support_ratio, support_off_gx, support_off_gy = compute_support(
                grid_x, grid_y, grid_z)

            pose_press_ready = deepcopy(posx1)

            if grid_z > 0 and 0.0 < support_ratio < 0.99:
                # 부분 지지: 누르는 위치를 '지지 중심'으로 이동
                off_x_mm = support_off_gx * LEGO_W_X
                off_y_mm = support_off_gy * LEGO_W_Y
                pose_press_ready[0] += off_x_mm
                pose_press_ready[1] += off_y_mm

                press_force = 25.0 * max(support_ratio, 0.4)
                stiffness_xy = 4000 + int((1.0 - support_ratio) * 8000)

                node.get_logger().info(
                    f"부분 지지 감지: 지지율={support_ratio:.2f}, "
                    f"누르기 위치 보정=({off_x_mm:+.1f}, {off_y_mm:+.1f})mm, "
                    f"힘={press_force:.1f}N, xy강성={stiffness_xy}"
                )
            else:
                press_force = 25.0
                stiffness_xy = 4000
            ...
            task_compliance_ctrl([stiffness_xy, stiffness_xy, 20000, 200, 200, 200], 0)
            wait(0.2)
