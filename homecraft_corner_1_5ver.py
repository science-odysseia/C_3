...
def main(args=None):
    ...
    try:
        from DSR_ROBOT2 import (
            ...
            get_current_posx, check_force_condition,
            DR_AXIS_X, DR_AXIS_Y
        )
        ...
        while rclpy.ok():
            ...
            for current_block in lego_queue:
                ...
                if use_slide:
                    ...
                    # 🔧 [접촉 감지] 고정된 SLIDE_TIME만큼 무조건 밀지 않고,
                    #    이웃 블록에 닿아 반발력이 커지는 순간을 감지해서 즉시 멈춘다.
                    #    - check_force_condition(axis, min, max, ref)는 힘의 "크기"가
                    #      [min, max] 범위 안에 있으면 True를 반환한다.
                    #    - 아직 접촉 전(자유 이동 중)이면 반발력이 작아 True,
                    #      접촉해서 반발력이 커지면 max를 넘어서 False가 된다.
                    #    - 즉, False가 되는 순간이 "접촉한 순간"이다.
                    CONTACT_FORCE_THRESHOLD = 3.0  # [N] 접촉으로 판단할 힘 임계값(현장에서 튜닝 필요)
                    t0 = time.time()
                    contacted = False
                    while time.time() - t0 < SLIDE_TIME:
                        # 실제로 미는 축만 검사 (밀지 않는 축은 항상 '접촉' 취급하여 무시)
                        x_ok = (slide_x == 0.0) or (not check_force_condition(
                            DR_AXIS_X, min=0.0, max=CONTACT_FORCE_THRESHOLD, ref=USER_COORD))
                        y_ok = (slide_y == 0.0) or (not check_force_condition(
                            DR_AXIS_Y, min=0.0, max=CONTACT_FORCE_THRESHOLD, ref=USER_COORD))
                        if x_ok and y_ok:
                            contacted = True
                            break
                        wait(0.05)
                    node.get_logger().info(
                        f"{'✅ 이웃 블록과 접촉 감지' if contacted else '⏱️ 타임아웃(접촉 미검출, SLIDE_TIME 종료)'}"
                    )
                    release_force(); release_compliance_ctrl(); wait(0.2)

                    # 🔧 [위치 보정] 컴플라이언스 힘제어로 밀린 뒤에는 그리퍼(=블록)가
                    #    이론상 계산값(posx1)이 아니라 "실제로 이웃과 접촉해서 멈춘 위치"에
                    #    가 있다. 이후 재파지/프레스 단계가 다시 이론값(posx1)으로 돌아가면
                    #    블록은 실제 위치에 그대로 있는데 그리퍼만 이동해버려 오차가 생긴다.
                    #    → 지금 그리퍼가 도달한 실제 좌표를 읽어와 posx1의 X, Y를 덮어쓴다.
                    actual_pose, _sol = get_current_posx(ref=USER_COORD)
                    posx1[0], posx1[1] = actual_pose[0], actual_pose[1]
                ...
                # 2️⃣ [패턴 B] (신규) 양쪽 다 막힌 사각형의 마지막 코너 자리에 꽂을 때: 비틀기 진입
                    ...
                    # 🔧 [접촉 감지] X축 반발력이 임계값을 넘는 순간(=이웃과 접촉) 바로 멈춤
                    CONTACT_FORCE_THRESHOLD_X = 4.0  # [N] 현장 튜닝 필요
                    t0 = time.time()
                    contacted_x = False
                    while time.time() - t0 < 1.2:
                        if not check_force_condition(DR_AXIS_X, min=0.0, max=CONTACT_FORCE_THRESHOLD_X, ref=USER_COORD):
                            contacted_x = True
                            break
                        wait(0.05)
                    node.get_logger().info(f"X축 {'접촉 감지' if contacted_x else '타임아웃'}")
                    release_force(); release_compliance_ctrl(); wait(0.1)

                    # 🔧 [위치 보정] X푸시로 실제 밀린 X좌표를 읽어와 posx1[0]에 반영.
                    #    이걸 안 하면 잠시 뒤 pose_y_ready[0] = posx1[0] 에서 다시
                    #    이론값으로 돌아가버려서, 블록을 쥔 채로 X방향으로 끌려가게 된다.
                    actual_pose_x, _sol = get_current_posx(ref=USER_COORD)
                    posx1[0] = actual_pose_x[0]

                    pose_x_escape = deepcopy(pose_x_down)
                    pose_x_escape[0] = actual_pose_x[0]
                    pose_x_escape[2] -= 25
                    ...
                    # 🔧 [접촉 감지] Y축 반발력이 임계값을 넘는 순간(=이웃과 접촉) 바로 멈춤
                    CONTACT_FORCE_THRESHOLD_Y = 4.0  # [N] 현장 튜닝 필요
                    t0 = time.time()
                    contacted_y = False
                    while time.time() - t0 < 1.2:
                        if not check_force_condition(DR_AXIS_Y, min=0.0, max=CONTACT_FORCE_THRESHOLD_Y, ref=USER_COORD):
                            contacted_y = True
                            break
                        wait(0.05)
                    node.get_logger().info(f"Y축 {'접촉 감지' if contacted_y else '타임아웃'}")
                    release_force(); release_compliance_ctrl(); wait(0.1)

                    # 🔧 [위치 보정] Y푸시까지 끝난 뒤 실제 도달 위치(X, Y 모두)를 읽어와
                    #    posx1을 갱신. 이후 pose_press_ready = deepcopy(posx1) 및
                    #    탈출 이동이 전부 이 실측값을 기준으로 움직이게 된다.
                    actual_pose_y, _sol = get_current_posx(ref=USER_COORD)
                    posx1[0], posx1[1] = actual_pose_y[0], actual_pose_y[1]

                    pose_y_escape = deepcopy(pose_y_down)
                    pose_y_escape[0], pose_y_escape[1] = actual_pose_y[0], actual_pose_y[1]
                    pose_y_escape[2] -= 25
