        ...
        while rclpy.ok():
            ...
            for current_block in lego_queue:
                ...
                # 2️⃣ [패턴 B] (신규) 양쪽 다 막힌 사각형의 마지막 코너 자리에 꽂을 때: 비틀기 진입
                    ...
                    node.get_logger().info("🚜 [불도저 모드] 블록을 가착한 후 손가락 외벽으로 밀어 넣습니다.")
                    
                    # 1. 원래 목표 위치보다 X축으로 4mm, Y축으로 4mm 떨어진 대각선 가착 위치 계산
                    pose_drop = deepcopy(posx1)
                    pose_drop[0] -= slide_x * SLIDE_OFFSET
                    pose_drop[1] -= slide_x * SLIDE_OFFSET
                    pose_drop[2] -= 14
                    
                    # 가착 위치로 이동 후 그리퍼 열어 블록 놓기
                    movel(pose_drop, vel=100, acc=ACC)
                    wait(0.2)
                    open_gripper()
                    wait(0.3)
                    
                    # 2. [중요] 그리퍼 손가락이 블록을 치지 않게 Z축으로 20mm 살짝 들어 올림
                    node.get_logger().info("손가락 들어 올리기")
                    pose_hover = deepcopy(pose_drop)
                    pose_hover[2] -= 20.0  # 안전 높이 후퇴
                    movel(pose_hover, vel=150, acc=ACC)
                    
                    # 3. 블록 뒤쪽(오프셋 외곽)으로 로봇 팔을 더 뒤로 뺌 (밀 준비)
                    node.get_logger().info("뒤로 빼기")
                    pose_push_start = deepcopy(pose_hover)
                    pose_push_start[0] -= slide_x * SLIDE_OFFSET  # 블록 뒤로 완전히 진입하기 위해 4mm 더 후퇴
                    pose_push_start[1] -= slide_x * SLIDE_OFFSET
                    movel(pose_push_start, vel=150, acc=ACC)
                    
                    # 4. 밀기 위해 다시 하강 (블록 허리를 밀 수 있는 높이로)
                    node.get_logger().info("다시 하강하기")
                    pose_push_down = deepcopy(pose_push_start)
                    pose_push_down[2] = posx1[2] - 4.0  # 블록 높이 중간 지점 타겟
                    node.get_logger().info("손가락 잠그기")
                    full_close_gripper()
                    movel(pose_push_down, vel=100, acc=ACC)
                    wait(0.1)
                    
                    # 5. [X축 푸시] 컴플라이언스를 켜고 X축으로 먼저 밀어버림
                    node.get_logger().info("x축으로 밀기")
                    ...
                    pose_final_press = deepcopy(posx1)
                    ...
                    move_periodic(amp=[0, 0, 0, 0, 0, 2], period=1.0, repeat=1, ref=USER_COORD); wait(0.5)
                    
                    # 루틴 종료 및 안전 탈출
                    release_force(); release_compliance_ctrl(); wait(0.2)
                    pose_escape = deepcopy(posx1)
                    pose_escape[2] -= 60.0
                    movel(pose_escape, vel=VELOCITY, acc=ACC)
