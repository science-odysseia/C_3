# lego_cad_web 패키지의 web_server_node.py로 부터 /centers 토픽을 구독해 토픽이 들어올 시 로봇팔을 작동시켜 해당하는 구조물을 건축하기 시작.
# 2일차에서 발전한 점 -> 토픽 구독해서 작동시키기, 블록 배치 안정확, 작동 완료시 대기상태로 돌아가기, 새로운 블록 입력및 /centers 토픽 발행시 이미 배치된 블록 기억해서 작업에서 배제하기.
    ...
    try:
        while rclpy.ok():
            ...
            for current_block in lego_queue:
                ...
                set_desired_force([0, 0, 25, 0, 0, 0], dir=[0, 0, 1, 0, 0, 0], mod=DR_FC_MOD_REL); wait(4.0)
                release_force(); release_compliance_ctrl(); wait(0.2)
