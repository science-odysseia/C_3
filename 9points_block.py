...
def main(args=None):
    ...
    # =========================================================================
     # =========================================================================
    # [4-2. 90도 회전 자세 place 전용 보정 -- 19점 룩업 테이블 방식]
    # =========================================================================
    # 블록을 문 상태 + 90도 회전 자세로 실측한 x라인(y=0) 19점.
    # key: 명목 x (= grid_x * 16), value: 실측 (x, y, z)
    # 티칭한 자리는 실측값 그대로, 사이 지점은 양옆 점 선형보간으로 사용.
    ROT_TABLE = {
    ...
    def apply_rotated_correction(px, py, grid_x, grid_y, grid_z):
        ...
        - grid_x가 티칭 지점(16의 배수 칸)과 일치하면 실측값 그대로 사용
        - 사이 지점은 양옆 두 점을 선형보간
        - y != 0 위치는 9점 보간의 y 변화량을 티칭 y값에 더해 근사
        """
        nominal_x = grid_x * LEGO_W_X

        # 테이블 범위 밖이면 끝점 값으로 클램프 (외삽 방지)
        ...
        # y=0 라인에서 딴 데이터이므로, y가 0이 아닌 위치는
        # 9점 보간이 계산한 y 변화량만큼 티칭 y에 더해 근사
        py_line0 = grid_to_pos(grid_x, 0, 0)[1]   # 같은 x, y=0에서의 9점 보간 y
        cy = ty + (py - py_line0)

        cz = tz - grid_z * LEGO_W_Z   # 층 반영 (102 z축 아래 방향)
        return tx, cy, cz
