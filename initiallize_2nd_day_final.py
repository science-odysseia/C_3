...
def main(args=None):
    ...
    PICK_POSE_X, PICK_POSE_Y, PICK_POSE_Z = 649, 53, 16
    PICK_FIX_A, PICK_FIX_B, PICK_FIX_C = 4.4, -179.15, 4.26
    # 고정된 레고 조립 회전 각도 (수직 자세)
    FIXED_A, FIXED_B, FIXED_C = 2.48, -178.23, 2.14
    ...
    pose_pick_ready[2] += 110  # 110mm 상공 대기 위치 계산
    ...
    try:
        ...
        for current_block in lego_queue:
            ...
            pose_pick_after = deepcopy(posepick)
            pose_pick_after[2] += 110
            # -----------------------------------------------------------------
            ...
            movel(pose_pick_after, vel=VELOCITY, acc=ACC)
            # -----------------------------------------------------------------
            # 조립 위치 진입 및 14번 확인 버튼 대기
            # -----------------------------------------------------------------
            pose_place_ready = deepcopy(posx1)
            pose_place_ready[2] += 110  # 30mm 상공 대기 위치 계산
