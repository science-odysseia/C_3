...
def cancel_monitor(navigator, monitor_node, nav_lock):
    while rclpy.ok() and not monitor_node.stop_requested:
        ...
        if monitor_node.tracking_requested:
            ...
            monitor_node.force_stop_robot(1.0)
            break

        if monitor_node.pause_requested:
...
def go_to_pose_with_interrupt(navigator, monitor_node, nav_lock, pose, name):
    while rclpy.ok():
        ...
        while rclpy.ok() and monitor_node.is_paused():
            ...
            monitor_node.force_stop_robot(0.2)
            time.sleep(0.05)
...
def shutdown_mission_only(navigator, monitor_node, monitor_executor, monitor_thread, cancel_thread):
...
class TrackingModeNode(Node):
    ...
    def make_goal_from_bottom(self, x, y, z, frame_id, K):
        ...
        # 도착 후 방향을 새로 맞추지 않도록 현재 로봇 방향을 그대로 사용
    ...
    def stop_robot(self):
        ...
        self.get_logger().info(
            'Stop distance reached. Navigation cancelled and follow mode enabled.'
        )
    ...
    def shutdown_after_search_fail(self):
        ...
        self.get_logger().warn(
            f'Target not found after {SEARCH_TURNS:.1f} turns. Shutdown.'
        )
    ...
    def start_final_search(self):
        ...
        try:
            ...
            self.get_logger().warn(f'Final search cancelTask failed: {e}')

        self.cmd_vel_pub.publish(Twist())

        self.get_logger().warn(
            'Arrived at first tracking goal but target is not detected. '
            'Start one-turn search before gohome_sector.'
        )
    ...
    def final_search_step(self):
        ...
        if rotated_angle >= FINAL_SEARCH_MAX_ROTATION:
            ...
            self.get_logger().warn(
                'One-turn search finished but target was not found. '
                '/robot3/gohome_sector=True and waiting.'
            )
            return
        ...
        self.get_logger().info(
            f'Final one-turn searching... rotated approx '
            f'{math.degrees(rotated_angle):.1f} / {math.degrees(FINAL_SEARCH_MAX_ROTATION):.1f} deg'
        )
    ...
    def process_bottom(self):
        ...
        if self.final_searching:
            ...
            self.get_logger().info(
                'Target detected during final one-turn search. Enter follow mode.'
            )
        ...
        if self.searching:
            ...
            self.get_logger().info('Target detected again. Resume follow mode.')
        ...
        if not (0.2 < z < 5.0):
            self.get_logger().warn(f'Invalid depth value at bottom: {z:.2f} m')
            return
        ...
        if self.follow_mode:
            if z <= STOP_DISTANCE:
                ...
                self.get_logger().info(
                    f'Follow mode: target within {STOP_DISTANCE:.2f} m. Holding stop.'
                )
                return
...
def main():
    ...
    navigator.info('Nearest point mission node started.')
    ...
    if mission_result == 'SUCCEEDED':
        navigator.info('Final target route completed.')
    else:
        navigator.info('Final target route failed.')
