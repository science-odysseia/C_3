...
class TrackingModeNode(Node):
    ...
    def shutdown_after_search_fail(self):
        ...
        self.cmd_vel_pub.publish(Twist())
        self.get_logger().warn(
            f'Target not found after {SEARCH_TURNS:.1f} turn. Start gohome.'
        )
    ...
    def search_without_initial_detection(self):
        now = self.get_clock().now()

        if not self.searching:
            self.search_direction = INITIAL_SEARCH_DIRECTION
            self.search_start_time = now
            self.searching = True
            self.get_logger().warn(
                'No initial detection in tracking mode. Start rotate search.'
            )
    ...
    def process_bottom(self):
        ...
        if not is_detected or bottom is None:
            ...
            if follow_mode:
                ...
                self.search_without_initial_detection()
        ...
        if self.follow_mode:
            ...
            try:
                goal_pose = self.make_goal_from_bottom(x, y, z, frame_id, K)
